// @mozilla/readability v0.6.0（Apache-2.0，Arc90 readability.js 衍生）
// 第一百八十五次（用户："回装 Readability，仅仅用在给AI提取正文"）：
//   从 npm 包 @mozilla/readability@0.6.0 原样取回，仅在文件末尾补一行 ESM 导出。
//   本文件不参与高亮/扫描路径，只被 lib/main-text.js 调用（给 AI 提取正文 + 诊断计时）。
//   注意：Readability.prototype.parse() 会直接改写传入的 document，故调用方必须传
//   DOMParser 解析出的独立副本，绝不能传真实页面 document（历史白屏教训）。
/*
