...
const DICT = {
  en: {
    ...
    'btn.showInPage': 'Show in page',
    // 309次：show in page 插入结果卡片内反馈行（panel.js showInsertFeedback）
    'th.insertOk': 'Inserted into page',
    'th.insertNoDef': 'Definition not ready',
    'th.insertNoAnchor': 'Page anchor lost, selection cleared',
    'th.insertInsideAnn': 'Anchor is inside an inserted translation; reselect original text',
    'th.insertFail': 'Insert failed',
...
'ws.micDenied': 'Microphone access denied (extension page). To reset:\n1. On this page click the icon left of the address bar → "Permissions for this site" → Microphone → Ask (or Reset permissions) → reload\n2. Or: edge://extensions → this extension → Details → "Reset permissions"\n3. Or: Windows Settings → Privacy → Microphone → allow apps.\nNote: reloading/reinstalling from the SAME folder keeps the permission; reloading from a DIFFERENT folder (new extension ID) resets it',
  ...
  zh: {
    ...
    // 309次：show in page 插入结果卡片内反馈行（panel.js showInsertFeedback）
    'th.insertOk': '已插入页面',
    'th.insertNoDef': '释义未就绪',
    'th.insertNoAnchor': '页面锚点已失效，选区已清除',
    'th.insertInsideAnn': '锚点在已插入译文/注释内，请重选原文',
    'th.insertFail': '插入失败',
